...
class Env_6(MultiAgentEnv):
    ...
    def step(self, action):
        ...
        if act == self.DoneAct or self.done[self.am.activate_agent - 1]:
            #print(f'player {self.am.activate_agent} done at step {self.steps}')
            self.done[self.am.activate_agent - 1] = True
            dist =  last_dist = self.compute_dist(self.am.activate_agent)[0]
            rewards= {f'agent_{self.am.activate_agent}': self.rs[self.am.activate_agent-1](0)[0]}
        else:

            row = act // self.chip.cols
            col = act % self.chip.cols
            #last_dist = self.distance_to_m(self.am.activate_agent)
            pre_pos = self.chip.get_p(self.am.activate_agent)
            last_dist  = self.compute_dist(self.am.activate_agent)[0]

            self.chip.goto(player=self.am.activate_agent, new_r=row, new_c=col)
            dist,all_dist,self_dist = self.compute_dist(self.am.activate_agent)

            if dist is None:
                terminateds = {"__all__": True}
                rewards = {f'agent_{self.am.activate_agent}': self.rs[self.am.activate_agent - 1](-1)[0]}
                # back to previous position
                #self.chip.goto(player=self.am.activate_agent, new_r=pre_pos[0], new_c=pre_pos[1])
                #self.done[self.am.activate_agent - 1] = True
            else:
                rewards = self.reward_function(dist=dist,last_dist=last_dist,all_dist = all_dist)
                self.sw[self.am.activate_agent - 1].next(dist)

        self.dist_rec[self.am.activate_agent - 1] = f'{last_dist}->{dist}'

        truncated = {}
        infos = self._get_infos()

        self.am.activate_agent = ((self.am.activate_agent) % self.num_qubits) + 1

        # switch to next player
        if not np.all(self.done):
            while self.done[self.am.activate_agent - 1]:
                self.am.activate_agent = ((self.am.activate_agent) % self.num_qubits) + 1
        self.steps += 1
    ...
    def _get_infos(self):
        return {
                    f'agent_{self.am.activate_agent}':
                    {
                        ...
                        'max_total_r':self.max_total_r[self.am.activate_agent - 1]
                    }
                 }
    ...
    def reward_function(self,dist,last_dist,all_dist):
        ...
        r = self.rs[self.am.activate_agent-1](r)[0]
        # update reward scaling

        for i in range(1, self.num_qubits + 1):
            if i == self.am.activate_agent:
